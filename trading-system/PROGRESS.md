...
> 마지막 갱신: 2026-06-15 (★★**돌파·평균회귀 1m/5m/1h 전부 OOS 엣지 없음 확정**. 1h INCONCLUSIVE를 OOS 3mo로 넓혀 재판정 → 거래수 확보하니 "유망하던" W2/W3 양전이 노이즈로 드러나며 **FAIL(평균 −50, 0/4)**. 타임프레임↑→손실↓(−136→−93→−50)이나 양전 안 됨. 라이브 불변). 다음 세션 이 파일부터 읽고 "🚀 다음 세션 첫 액션". ★★다음=**돌파/평균회귀 계열 종료 → 엔진버퍼 확장(정통 일봉) or 다른 데이터(오더북/펀딩비) or 여기서 멈춤** — 사용자 결정 대기.
...
### ⏭ 다음 세션 — ★돌파/평균회귀 계열 종료 → 새 방향 (사용자 결정 대기)
**돌파·평균회귀가 1m/5m/1h 전부 FAIL. 롤링 캔들기반·BTC+ETH 전략은 이 데이터에서 소진됨.**

다음 첫 액션 후보(브레인스토밍/사용자 결정):
1. **엔진 버퍼 확장**(`PAPER_MAX_CANDLES_PER_KEY`↑ + 시작 DB백필) → 정통 일봉 래리윌리엄스(1d 캔들). ⚠️**`BACKTEST_HISTORY_LIMIT=64`** → 현재 5m/1h도 lookback≤~63캔들(1h×64=2.7일)이라 일봉 가려면 버퍼/limit 확장 필수(엔진 수정=범위 커짐). 1d 캔들은 1m 롤업으로 만들 수 있음(`floor(epoch/86400)`).
2. **다른 데이터**(오더북 불균형/펀딩비) — `Strategy::evaluate`가 캔들만 받으므로 trait 확장 필요.
3. **여기서 멈춤** — "쉽게 찾을 OOS 엣지 없음"이 정직한 결론. 검증 인프라(워크포워드+수수료+3진판정+적대적리뷰)는 완비됨, 향후 가설 생기면 재사용.
- ⚠️ 보장 없음. **family-wise 과최적화 주의**(같은 BTC/ETH·같은 거시국면 반복 시도=거짓양성 누적). 마진 PASS면 한 번도 안 쓴 새 홀드아웃 필수.
