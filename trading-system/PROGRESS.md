...
> **🟡 비차단 관찰 — Binance WS 단독 지연(돈위험 0, latency-gate 정상작동)**: testnet 봇 가동 중 **Binance 피드만** latency 2~4초로 올라(bitget/bybit는 정상 13s) latency-gate가 진입 전면 차단(`market_data_latency` block 425건, behind 104→186s 증가=자가복구 안 됨, reconnect 라인 고정). **이게 메모리 박제 "WS partial-stall"일 가능성**(오더북류 프레임만 죽고 저빈도는 살아 idle-timeout 안 걸림). **봇 재시작으로 즉시 해소**(behind 정상·block 0). ⚠️**게이트는 의도대로 작동**(오래된 가격 진입 차단=돈 안전). per-stream staleness 재연결(`6056de3`)이 이 케이스를 못 잡았을 수 있음 — 캐처(Hetzner)엔 같은 수정 배포됨, 라이브 봇 트랙에선 재발 시 재조사 필요(후속, 비차단).
